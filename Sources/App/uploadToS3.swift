...
internal func uploadToS3(bucket: String, fileName: String, data: Data) async throws {
    let serviceHandler = await ServiceHandler()
    try await serviceHandler.uploadFile(bucket: bucket, key: fileName, data: data)
}
...
private class ServiceHandler {
    ...
    public init() async {
        do {
            client = try S3Client(region: "eu-central-1")
        } catch {
    ...
    public func uploadFile(bucket: String, key: String, file: String) async throws {
        ...
        try await uploadFile(bucket: bucket, key: key, data: fileData)
    }

    public func uploadFile(bucket: String, key: String, data: Data) async throws {
